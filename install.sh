...
echo -e "${PURPLE}====================================================${NC}"
echo -e "${PURPLE}      DAC Panel v4.1 - XUI Path Bug Fixed           ${NC}"
echo -e "${PURPLE}====================================================${NC}"
...
echo -e "${CYAN}[1/6] Downloading Local Fonts...${NC}"
...
echo -e "${CYAN}[2/6] Configuring Backend (Dynamic XUI Path)...${NC}"

cat << 'EOF' > server/package.json
{
  "name": "dac-server", "version": "4.1.0", "type": "module",
...
// TEST CONNECTION (جدید)
app.post('/api/settings/test-xui', async (req, res) => {
    try {
        const { url, username, password } = req.body;
        let baseUrl = url.replace(/\/+$/, ""); // حذف اسلش آخر برای جلوگیری از دوگانه شدن
        const r = await axios.post(`${baseUrl}/login`, { username, password });
        if (r.data.success) res.json({ success: true, message: 'اتصال با موفقیت برقرار شد!' });
        else res.status(401).json({ success: false, message: 'نام کاربری یا رمز عبور x-ui اشتباه است' });
    } catch (e) {
        res.status(500).json({ success: false, message: `خطا در اتصال: ${e.message.includes('ENOTFOUND') ? 'آدرس سرور پیدا نشد' : 'مسیر پنل اشتباه است یا پورت بسته است'}` });
    }
});
...
app.post('/api/agents/buy-package', (req, res) => {
    ...
    if (!pkg || !agent) return res.status(404).json({error: 'یافت نشد'});
    if (agent.balance < pkg.price_agent) return res.status(400).json({error: 'موجودی نماینده کافی نیست'});
...
// X-UI API WRAPPER (اصلاح شده برای پشتیبانی از مسیرهای متغیر)
async function getXui() {
    ...
    let baseUrl = s.url.replace(/\/+$/, ""); // حذف اسلش‌های اضافی آخر آدرس
    const res = await axios.post(`${baseUrl}/login`, { username: s.username, password: dec(s.password) });
    const cookie = res.headers['set-cookie'].find(c => c.startsWith('session='))?.split(';')[0];
    if(!cookie) throw new Error("Cookies failed");
    // دیگر نیازی به اضافه کردن /panel/ نیست چون کاربر آدرس کامل را داده است
    return { baseURL: baseUrl, headers: { Cookie: cookie, 'Content-Type': 'application/json', 'Accept': 'application/json' } };
}

app.get('/api/data', async (req, res) => {
    try {
        const xui = await getXui();
        // حذف شده /panel/ از اینجا: ${xui.baseURL}/inbound/list
        const ibs = (await axios.get(`${xui.baseURL}/inbound/list`, { headers: xui.headers })).data.obj || [];
        let users = [];
        ibs.forEach(ib => {
...
app.post('/api/users/create', async (req, res) => {
    try {
        ...
        const ib = (await axios.get(`${xui.baseURL}/inbound/list/${inboundId}`, { headers: xui.headers })).data.obj;
        ib.clients.push(nu);
        await axios.post(`${xui.baseURL}/inbound/update/${inboundId}`, ib, { headers: xui.headers });
...
app.listen(PORT, '0.0.0.0', () => console.log(`\n🚀 DAC Panel v4.1 running on port ${PORT}\n`));
SERVERCODE
...
{
  "name": "dac-ui", "private": true, "version": "4.1.0", "type": "module",
...
export const Modal = ({ open, onClose, children }) => (
  <AnimatePresence>{open && (<><motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 bg-black/60 backdrop-blur-sm z-40" onClick={onClose}/><motion.div initial={{scale:0.9, opacity:0}} animate={{scale:1, opacity:1}} exit={{scale:0.9, opacity:0}} className="fixed inset-0 m-auto w-[95vw] max-w-md max-h-[90vh] overflow-y-auto bg-slate-900/90 backdrop-blur-xl border border-white/10 rounded-2xl p-6 z-50 shadow-2xl">{children}</motion.div></>)}</AnimatePresence>
);
export const Drawer = ({ open, onClose, user }) => (
  <AnimatePresence>{open && user && (<><motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 bg-black/60 backdrop-blur-sm z-40" onClick={onClose}/><motion.div initial={{x:500}} animate={{x:0}} exit={{x:500}} transition={{type:"spring", damping:25}} className="fixed left-0 top-0 h-full w-full max-w-md bg-slate-900/90 backdrop-blur-xl border-l border-white/10 z-50 p-6 shadow-2xl overflow-y-auto">{children}</motion.div></>)}</AnimatePresence>
);
COMPONENTS

echo -e "${CYAN}[4/6] Building Main App Logic...${NC}"
...
import { LayoutDashboard, Users, UserPlus, UserCog, Settings, LogOut, Menu, X, Plus, RefreshCw, Ban, Trash2, Wifi, Save, Shield, Package, Megaphone, Github, Zap } from 'lucide-react';
...
export default function App() {
    ...
    // پیام هوشمند (سبز برای موفقیت، قرمز برای خطا)
    ...
    const doAction = async (fn, id='global') => { setLoading(p => ({...p, [id]: true})); try { return await fn(); } finally { setLoading(p => ({...p, [id]: false})); } };
    ...
    const saveXui = async () => { await doAction(async () => { await api.post('/settings/xui', xuiSettings); setMsg({text:'تنظیمات ذخیره شد', type:'success'}); setTimeout(()=>setMsg({text:'', type:''}), 3000); fetchAll(); }, 'saveXui'); };
    
    const changePass = async () => { await doAction(async () => { try { await api.post('/auth/change-password', passForm); setMsg({text:'رمز تغییر کرد', type:'success'}); setPassForm({oldPassword:'', newPassword:''}); } catch(e) { setMsg({text:e.response.data.error, type:'error'}); } setTimeout(()=>setMsg({text:'', type:''}), 3000); }, 'changePass'); };
    ...
    const testXui = async () => {
        await doAction(async () => {
            try {
                const r = await api.post('/settings/test-xui', xuiSettings);
                setMsg({text: r.data.message, type: 'success'});
            } catch(e) {
                setMsg({text: e.response?.data?.message || 'خطای ناشناخته', type: 'error'});
            }
    ...
    if (!loggedIn) return (
        <div className="min-h-screen bg-animated flex items-center justify-center p-4">
            <motion.form onSubmit={login} initial={{opacity:0, y:50}} animate={{opacity:1,y:0}} className="w-full max-w-md bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-8 shadow-2xl">
                ...
                <input required placeholder="نام کاربری (admin)" value={auth.username} onChange={e=>setAuth({...auth, username:e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-white mb-4 focus:outline-none focus:border-purple-500"/>
                <input required type="password" placeholder="رمز عبور (admin)" value={auth.password} onChange={e=>setAuth({...auth, password:e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-white mb-6 focus:outline-none focus:border-purple-500"/>
                <button type="submit" disabled={loading.login} className="w-full py-3 rounded-xl bg-gradient-to-l from-purple-600 to-blue-600 text-white font-bold hover:opacity-90 transition flex items-center justify-center gap-2">{loading.login ? <Loading/> : <Shield size={18}/>} ورود</button>
            </motion.form>
        </div>
    );
    ...
    return (
        <div className="flex min-h-screen font-vazir text-white overflow-hidden relative">
            ...
            <motion.aside initial={{x:300}} animate={{x: side ? 0 : (window.innerWidth > 768 ? 0 : 300)}} className="fixed md:relative z-30 w-72 h-screen p-4 flex flex-col gap-2 bg-white/5 backdrop-blur-xl border-l border-white/10">
                ...
                <a href="https://github.com/rezajakson/dac-panel" target="_blank" className="flex items-center gap-2 p-3 text-gray-500 hover:text-gray-300 transition text-xs mt-auto border-t border-white/10 pt-4"><Github size={14}/> GitHub</a>
            ...
            <main className="flex-1 relative z-10 p-6 md:p-10 overflow-y-auto">
                ...
                {page === 'dashboard' && (
                    <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} className="space-y-6">
                        ...
                        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-xl p-4">
                            <h3 className="font-bold text-yellow-400 flex items-center gap-2 mb-4"><Megaphone size={18}/> اطلاعیه‌ها</h3>
                            ...
                            <div className="flex gap-2"><input value={annText} onChange={e=>setAnnText(e.target.value)} placeholder="متن اطلاعیه..." className="flex-1 bg-white/5 border border-white/10 rounded-lg p-2 text-sm text-white focus:outline-none"/><button onClick={postAnn} disabled={loading.postAnn} className="px-4 bg-yellow-500/20 text-yellow-300 rounded-lg text-sm">{loading.postAnn ? <Loading/> : 'ارسال'}</button></div>
                        </div>
                    </motion.div>
                )}
                ...
                {page === 'packages' && (
                    <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} className="space-y-4">
                        <div className="flex justify-between items-center"><h2 className="text-2xl font-bold">مدیریت سرویس‌ها</h2>
                ...
                {page === 'settings' && (
                    <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} className="max-w-3xl mx-auto space-y-8">
                        ...
                        <AnimatePresence>
                            {msg.text && (
                                <motion.div initial={{opacity:0, y:-10}} animate={{opacity:1, y:0}} exit={{opacity:0}} className={`border p-3 rounded-xl text-center text-sm ${msg.type === 'success' ? 'bg-green-500/20 border-green-500/30 text-green-300' : 'bg-red-500/20 border-red-500/30 text-red-300'}`}>
                                    {msg.text}
                                </motion.div>
                            )}
                        </AnimatePresence>
                        ...
                        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6">
                            ...
                            <div className="space-y-4">
                                <div>
                                    <label className="text-sm text-gray-300 block mb-1">آدرس کامل پنل (شامل مسیر مخفی)</label>
                                    <input dir="ltr" placeholder="http://ip:port/secret_path/panel" value={xuiSettings.url} onChange={e=>setXuiSettings({...xuiSettings, url:e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-white focus:outline-none focus:border-purple-500 text-left"/>
                                </div>
                                <div className="grid grid-cols-2 gap-4">
                                    <div><label className="text-sm text-gray-300 block mb-1">نام کاربری x-ui</label><input value={xuiSettings.username} onChange={e=>setXuiSettings({...xuiSettings, username:e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-white focus:outline-none focus:border-purple-500"/></div>
                                    <div><label className="text-sm text-gray-300 block mb-1">رمز عبور x-ui</label><input type="password" value={xuiSettings.password} onChange={e=>setXuiSettings({...xuiSettings, password:e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-white focus:outline-none focus:border-purple-500"/></div>
                                </div>
                                <div className="grid grid-cols-2 gap-4">
                                    <motion.button whileTap={{scale:0.95}} onClick={testXui} disabled={loading.testXui} className="py-3 rounded-xl bg-cyan-600/30 text-cyan-200 border border-cyan-500/30 hover:bg-cyan-600/50 transition flex items-center justify-center gap-2">{loading.testXui ? <Loading/> : <Zap size={18}/>} تست اتصال</motion.button>
                                    <motion.button whileTap={{scale:0.95}} onClick={saveXui} disabled={loading.saveXui} className="py-3 rounded-xl bg-purple-600/30 text-purple-200 border border-purple-500/30 hover:bg-purple-600/50 transition flex items-center justify-center gap-2">{loading.saveXui ? <Loading/> : <Save size={18}/>} ذخیره</motion.button>
                                </div>
                            </div>
                        </div>
            ...
            <Modal open={modal.open} onClose={()=>setModal({open:false, type:''})}>
                {modal.type === 'user' && (<div className="space-y-4"><h2 className="text-xl font-bold text-purple-400">ساخت کاربر جدید</h2><div><label className="text-sm text-gray-300 block mb-1">نام کاربری</label><input value={uForm.name} onChange={e=>setUForm({...uForm, name:e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-white"/></div><div><label className="text-sm text-gray-300 block mb-1">اینباند</label><select value={uForm.inboundId} onChange={e=>setUForm({...uForm, inboundId:e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-white">{data.inbounds.map(ib=><option key={ib.id} value={ib.id} className="bg-slate-800">{ib.tag} ({ib.protocol})</option>)}</select></div><div className="grid grid-cols-2 gap-4"><div><label className="text-sm text-gray-300 block mb-1">مدت زمان (روز)</label><input type="number" value={uForm.daysLimit} onChange={e=>setUForm({...uForm, daysLimit:e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-white"/></div><div><label className="text-sm text-gray-300 block mb-1">حجم (گیگابایت)</label><input type="number" value={uForm.gbLimit} onChange={e=>setUForm({...uForm, gbLimit:e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-white"/></div></div><label className="flex items-center gap-2 text-sm text-gray-300 cursor-pointer"><input type="checkbox" checked={uForm.startOnFirstConnect} onChange={e=>setUForm({...uForm, startOnFirstConnect: e.target.checked})} className="accent-purple-500"/>شروع از اولین اتصال</label><div><label className="text-sm text-gray-300 block mb-1">ساخت به عنوان</label><select value={uForm.agentId} onChange={e=>setUForm({...uForm, agentId:e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-white"><option value="" className="bg-slate-800">مدیر سیستم</option>{agents.map(a=><option key={a.id} value={a.id} className="bg-slate-800">{a.name}</option>)}</select></div><button onClick={mkUser} disabled={loading.mkUser} className="w-full py-3 rounded-xl bg-gradient-to-l from-purple-600 to-blue-600 text-white font-bold">{loading.mkUser ? <Loading/> : 'ساخت کاربر'}</button></div>)}
                {modal.type === 'pkg' && (<div className="space-y-4"><h2 className="text-xl font-bold text-cyan-400">ساخت سرویس جدید</h2><div><label className="text-sm text-gray-300 block mb-1">نام سرویس</label><input value={pForm.name} onChange={e=>setPForm({...pForm, name:e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-white"/></div><div className="grid grid-cols-2 gap-4"><div><label className="text-sm text-gray-300 block mb-1">روز</label><input type="number" value={pForm.days_limit} onChange={e=>setPForm({...pForm, days_limit:e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-white"/></div><div><label className="text-sm text-gray-300 block mb-1">حجم (GB)</label><input type="number" value={pForm.gb_limit} onChange={e=>setPForm({...pForm, gb_limit:e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-white"/></div></div><div className="grid grid-cols-2 gap-4"><div><label className="text-sm text-gray-300 block mb-1">قیمت فروش (تومان)</label><input type="number" value={pForm.price_user} onChange={e=>setPForm({...pForm, price_user:e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-white"/></div><div><label className="text-sm text-gray-300 block mb-1">قیمت نماینده (تومان)</label><input type="number" value={pForm.price_agent} onChange={e=>setPForm({...pForm, price_agent:e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-white"/></div></div><button onClick={mkPackage} disabled={loading.mkPack} className="w-full py-3 rounded-xl bg-gradient-to-l from-cyan-600 to-blue-600 text-white font-bold">{loading.mkPack ? <Loading/> : 'ثبت سرویس'}</button></div>)}
                {modal.type === 'agent' && (<div className="space-y-4"><h2 className="text-xl font-bold text-blue-400">افزودن نماینده</h2><div><label className="text-sm text-gray-300 block mb-1">نام</label><input value={aForm.name} onChange={e=>setAForm({...aForm, name:e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-white"/></div><div className="grid grid-cols-2 gap-4"><div><label className="text-sm text-gray-300 block mb-1">یوزرنیم</label><input value={aForm.username} onChange={e=>setAForm({...aForm, username:e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-white"/></div><div><label className="text-sm text-gray-300 block mb-1">رمز</label><input value={aForm.password} onChange={e=>setAForm({...aForm, password:e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-white"/></div></div><div className="grid grid-cols-2 gap-4"><div><label className="text-sm text-gray-300 block mb-1">شماره</label><input value={aForm.phone} onChange={e=>setAForm({...aForm, phone:e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-white"/></div><div><label className="text-sm text-gray-300 block mb-1">اینباند پیش‌فرض</label><select value={aForm.default_inbound_id} onChange={e=>setAForm({...aForm, default_inbound_id:e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-white"><option value="" className="bg-slate-800">ندارد</option>{data.inbounds.map(ib=><option key={ib.id} value={ib.id} className="bg-slate-800">{ib.tag}</option>)}</select></div></div><button onClick={mkAgent} disabled={loading.mkAgent} className="w-full py-3 rounded-xl bg-gradient-to-l from-blue-600 to-cyan-600 text-white font-bold">{loading.mkAgent ? <Loading/> : 'ثبت نماینده'}</button></div>)}
            </Modal>
        </div>
    );
}
...
echo -e "${CYAN}[5/6] Building Project...${NC}"
cd client
...
clear
echo -e "${GREEN}====================================================${NC}"
echo -e "${GREEN}     ✅ DAC Panel v4.1 (Path Fixed) Installed!       ${NC}"
echo -e "${GREEN}====================================================${NC}"
echo -e "${NC}Login: ${PURPLE}admin${NC} / ${PURPLE}admin${NC}"
echo -e "Run: ${PURPLE}pm2 start /root/dac-panel/server/index.js --name dac${NC}"
